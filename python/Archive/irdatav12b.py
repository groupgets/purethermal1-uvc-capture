...
from PyQt4.QtGui import (QImage, QWidget, QApplication, QLabel, QPixmap, QPushButton, QVBoxLayout, QGridLayout, QSizePolicy, QMessageBox)
import sys
import cv2
...
def startStream():
    ...
    if res < 0:
      print("Find Device Error... Most Likely Not Plugged In")
      exit(1)
...
def getFrame():
    ...
    display_temperatureK(img, minVal, minLoc, (255, 0, 0))
    display_temperatureK(img, maxVal, maxLoc, (0, 0, 255))
    #print(minLoc)
    display_temperatureK(img, minVal, (10,55), (255, 0, 0))
    display_temperatureK(img, maxVal, (10,25), (0, 0, 255))
    return img
...
def stopRecAndSave():
    ...
    if tiff_frame > 1:
	    ...
	    camState = 'not_recording'
	    try:
		imsave('Lepton_Vid_' + `fileNum` + '.tiff',dataCollection)
		print('Saved Content to File Directory')
		fileNum += 1
	    except:
		print('Save Failed')
            x = np.zeros(dataCollection.shape)
            dataCollection = x[:,:,0]
	    tiff_frame = 1
